        ...
        VStack {
            Form {
                ...
                HStack(spacing: 30) {
                    Spacer(minLength: 0)
                    Button {
                        if let url = URL(string: "https://github.com/SapientialM/What-To-Do") {
...
struct AISettings: View {
    @Default(.deepSeekAPIKey) var apiKey
    @Default(.deepSeekEndpoint) var endpoint
    @Default(.deepSeekModel) var model
    ...
    var body: some View {
        Form {
            Section {
                ...
                Text("默认使用 OpenAI 兼容的 DeepSeek API 端点。如需使用其他兼容接口，请修改端点地址。")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
            ...
            Section {
                HStack {
                    Button(action: {
                        ...
                        testResult = nil
                        Task {
                            let (success, message) = await ChatViewModel.shared.testConnection()
                            await MainActor.run {
